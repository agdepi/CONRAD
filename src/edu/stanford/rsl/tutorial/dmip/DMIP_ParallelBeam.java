...
public class DMIP_ParallelBeam {
	...
	/**
	 * Forward projection of the phantom onto the detector Rule of thumb: Always
	 * sample in the domain where you expect the output! Thus, we sample at the
	 * detector pixel positions and sum up the informations along one ray
	 * 
	 * @param grid
	 *            the image
	 * @param maxTheta
	 *            the angular range in radians
	 * @param deltaTheta
	 *            the angular step size in radians
	 * @param maxS
	 *            the detector size in [mm]
	 * @param deltaS
	 *            the detector element size in [mm]
	 */

	public Grid2D projectRayDriven(Grid2D grid, double maxTheta,
			double deltaTheta, double maxS, double deltaS) {
		...
		Grid2D sino = new Grid2D(new float[maxThetaIndex * maxSIndex],
				maxSIndex, maxThetaIndex);
		...
		Box b = new Box((grid.getSize()[0] * grid.getSpacing()[0]),
		...
		for (int e = 0; e < maxThetaIndex; ++e) {
			...
			for (int i = 0; i < maxSIndex; ++i) {
				...
				for (double t = 0.0; t < distance * samplingRate; ++t) {
					...
					double x = current.get(0) / grid.getSpacing()[0], y = current
							.get(1) / grid.getSpacing()[1];

					if (grid.getSize()[0] <= x + 1
							|| grid.getSize()[1] <= y + 1 || x < 0 || y < 0)
						continue;
	...
	/**
	 * Sampling of projections is defined in the constructor. Backprojection of
	 * the projections/sinogram Rule of thumb: Always sample in the domain where
	 * you expect the output! Here, we want to reconstruct the volume, thus we
	 * sample in the reconstructed grid! The projections are created pixel
	 * driven!
	 * 
	 * @param sino
	 *            the sinogram
	...
	public Grid2D backprojectPixelDriven(Grid2D sino, int imageSizeX,
			int imageSizeY, float pxSzXMM, float pxSzYMM) {
		...
		grid.setOrigin(-(grid.getSize()[0] * grid.getSpacing()[0]) / 2,
				-(grid.getSize()[1] * grid.getSpacing()[1]) / 2);
		...
		NumericPointwiseOperators.divideBy(grid,
				(float) (maxThetaIndex / Math.PI));
		return grid;
	}
	 ...
	 * 
	 * The ramp filters are defined in the spatial domain but they are applied
	 * in the frequency domain. Remember: a convolution in the spatial domain
	 * corresponds to a multiplication in the frequency domain.
	 * 
	 * Both, the sinogram and the ramp filter are transformed into the frequency
	 * domain and multiplied there.
	 * 
	 * @param sinogram
	 *            a line of the sinogram
	 * 
	 */
	public Grid1D rampFiltering(Grid1D sinogram, RampFilterType filter) { // sinogram
																			// =
																			// just
																			// a
																			// line
		...
		if (filter == RampFilterType.RAMLAK) {
			...
			for (int i = paddedSize / 2; i < paddedSize; i++) { // shift
		...
		for (int p = 0; p < sinogramF.getSize()[0]; p++) {
			sinogramF.multiplyAtIndex(p, ramp.getAtIndex(p),
					ramp.getImagAtIndex(p));
		}
	...
	public static void main(String[] args) {
		...
		Grid2D sinogram = parallel.projectRayDriven(sheppLoganPhantom,
				angularRange - angularStepSize, angularStepSize, detectorSize,
				detectorSpacing);
		...
		for (int theta = 0; theta < sinogram.getSize()[1]; ++theta) {
			...
			Grid1D tmp = parallel.rampFiltering(sinogram.getSubGrid(theta),
					filter);
		...
		Grid2D reco = parallel.backprojectPixelDriven(filteredSinogram,
				phantomSize, phantomSize, detectorSpacing, detectorSpacing);
