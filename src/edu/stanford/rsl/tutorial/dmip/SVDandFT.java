...
public class SVDandFT {
	...
	public static void invertSVD(SimpleMatrix A)
	{
		...
		DecompositionSVD svd = new DecompositionSVD (A);	 // takes matrix as a input
		...
		// go over ll diagonal elements and invert them
		...
		System.out.println("Srec = " +svd.getreciprocalS().toString()); // gives the inverse of the svd
		...
		for(int i = 0; i < sInd; i++)
		{
			...
			if (val > eps)
			{
				Slowrank.setElementValue(i, i, val); // check the diagonal
			}
		}
	...
	public static void optimizationProblem1(SimpleMatrix A, int rankDeficiency)
	{
		...
		for (int i = 0; i < newRank; i++)
		{
			Slowrank.setElementValue(i, i, mean); // calculated the mean
		}
	...
	public static void optimizationProblem4(double[] xCoords, double[] yCoords)
	{
		...
		//get solution for b
		SimpleMatrix Ainv = A.inverse(InversionType.INVERT_SVD);//initialisiseren der matrix - gives pseudoinverse
	...
	public static void optimizationProblem2(SimpleMatrix b)
	{
		...
		SimpleVector a = svd.getV().getCol(lastCol); // last column is a vector
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/i5dmip/gy27waki/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/yu_fill.jpg";
		...
		String filenameShepp = "/proj/i5dmip/gy27waki/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/Shepp-Logan Phantom.tif";
